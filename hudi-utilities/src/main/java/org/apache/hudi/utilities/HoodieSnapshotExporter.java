...
public class HoodieSnapshotExporter {
  ...
  private void exportAsNonHudi(JavaSparkContext jsc, Config cfg, List<String> partitions, String latestCommitTimestamp) {
    ...
    Dataset<Row> sourceDataset = SparkSession.builder().sparkContext(jsc.sc()).getOrCreate().sqlContext().read().parquet(JavaConversions.asScalaIterator(exportingFilePaths).toSeq());
